  ...
  async getAllProducts() {
    try {
      ...
      return {
        status: 'Fail',
        code: 500,
        message: `Internal Server Error getAllProducts ${error}`,
        payload: {},
      }
    }
  }
  async getCategories() {
    try {
      ...
      return {
        status: 'Fail',
        code: 500,
        message: `Internal Server Error getCategories ${error}`,
        payload: {},
      }
    }
  }
  async getProductsFilter({ limit, page, sort, query, baseUrl }) {
    ...
    try {
      ...
      return {
        ...
        hasPrevPage,
        hasNextPage,
        prevLink: hasPrevPage ? `${dataConfig.url_api_local}${baseUrl}?limit=${limitValue}&query=${encodeURIComponent(query)}&page=${pageNumber - 1}` : null,
        nextLink: hasNextPage ? `${dataConfig.url_api_local}${baseUrl}?limit=${limitValue}&query=${encodeURIComponent(query)}&page=${pageNumber + 1}` : null,
      }
    } catch (error) {
      return {
        status: 'Fail',
  ...
  async getProductByID(_id) {
    try {
      ...
      return {
        status: 'Fail',
        code: 500,
        message: `Internal Server Error getProductByID ${error}`,
        payload: {},
      }
    }
  }
  async getProductByCode(code) {
    try {
      ...
      return {
        status: 'Fail',
        code: 500,
        message: `Internal Server Error getProductByCode ${error}`,
        payload: {},
      }
    }
  }
  async getProductsByName(name) {
    try {
      ...
      return {
        status: 'Fail',
        code: 500,
        message: `Internal Server Error getProductsByName ${error}`,
        payload: {},
      }
    }
  }
