  ...
  return (
    <div className='mb-2'>
      <label className='block text-xs lg:text-base xxl:text-lg font-semibold text-gray-600'>
        {label}:
        <select name={name} disabled={!editable} value={itemValue} onChange={handleFieldChange} className={`w-full h-9 px-2 mt-1 text-sm py-1 border rounded-md focus:outline-red-200 ${className}`}>
